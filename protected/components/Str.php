...
class Str
{
     ...
     * @return string 
     */
    static function formatNumber($number, $type = 0, $decimals = 0, $des_point = ',',
        $thousands_sep = '.')
    {
    ...
    public static function makeLink($id, $title, $prefix = '', $suffix = '')
    {
        if ($prefix != '')
            $prefix = '/' . $prefix . '/';
        return $prefix . CVietnameseTools::makeUrlFriendly($title) . '-' . $id . '.html';
    }
    ...
    public static function encode_aes_file($file, $create_new_file = false)
    {
        ...
        if ($handle)
        {
            while (($buffer = fgets($handle, 4096)) !== false)
            {
                ...
                {
                    $content .= GibberishAES::enc($buffer, $file_name);
                    $content .= "\n";
                } else
                {
                    $content .= $buffer;

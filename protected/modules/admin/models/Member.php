...
class Member extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
         ...
         array('password', 'compare', 'compareAttribute'=>'repeat_password','message' => 'Xác nhận mật khẩu không chính xác','on' => 'insert'),
            array('is_admin, status, gender, group_id', 'numerical', 'integerOnly'=>true),
            ...
            array('id, display_name, user_name, password, email, phone_number, is_admin, address, status, gender, birthday, createdate, group_id', 'safe', 'on'=>'search'),
        );
    }
   ...
   /**
    * Tra ve danh sach tai khoan theo nhom user
   */
   public static function getUserByGroup($label = 'sp_admin'){
      $data = array();
      switch($label){
         case 'sp_admin';
            $group_id = 1;
      ...
      $sql = "SELECT user_name FROM member WHERE group_id = $group_id";
      $db = Yii::app()->db;
